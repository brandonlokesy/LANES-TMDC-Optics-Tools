...
**A12. `_order_by_iter` cannot see duplicate iteration indices.** — *open, reported
2026-08-07* The gap check, `set(range(seen[0], seen[-1] + 1)) - set(seen)`, is silent
on duplicates by construction — it compares against the *set* of indices, which a
repeat leaves unchanged — and CPython's stable sort leaves a colliding pair in glob
order. So two files claiming the same point load silently, the count exceeds the
number of distinct points, and every point from the collision onward is offset
against a per-point variable array.

Real exports do produce collisions: every committed real-space directory holds a
timestamped spectral file ending `_iter_0` beside a frame `_iter_0000`, and the TRPL
metadata companion collides on `iter_0` with the first decay. Both are excluded by
content before the sort, so neither reaches it today.

*Fix:* group the sorted files by index and warn when any index has more than one,
naming the colliding files rather than counting them. Whether it is worth the code
turns on whether the state is reachable with every file legitimate — check that
before deciding.
...
1. **A5**, **A9** — the remaining live bugs, both small and independently testable.
   ...
   (duplicate iteration indices) were opened by the A7 work: A11 is its own
   deliberate pass over 15 sites, A12 wants its reachability settled first.
