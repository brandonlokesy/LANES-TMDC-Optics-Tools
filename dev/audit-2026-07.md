...
with E14; **A7 and A9 open, reported not fixed** — both are
`AttoCubePLScanRealSpace` file discovery and want one pass together. **A10 open,
reported 2026-08-06**.)*
...
*Fixed 2026-08-06, with **E14**.* `_warn_if_sweep_is_sawtooth`, called at the end
of `_bind_nesting` because that is where both facts are known, warns when the
resolved sweep axis is non-monotonic **and** a nest is declared or detected. It
names the structure and points at `as_grid()` / `get_spectrum_at()`. Only the
sawtooth case warns: `sweep="piezo_y"` on the same raster gives a monotonic
staircase, which overplots less catastrophically and is left alone rather than
folded in unasked — see the *adjacent* note in E14.

The grid API it calls "the proper fix" is E14, which arrived with a different
spelling: `fast_sweep=` / `slow_sweep=`, not `grid=(inner, outer)`. Reasons there.

*Test:* `tests/test_loaders_nesting.py`, 4 cases — the warning fires and names
the grid, a monotonic sweep on the same file is silent, an undeclared sweep is
...
anti-symmetric gate sweep is.** — *open, reported 2026-08-06* **[verified by
running]** `loaders.py:2282` ranks rows by span relative to their own magnitude:
...
*Fix:* scale by something that cannot be zero for a row that varies. The span
itself is the obvious candidate — `span / max(abs(mean), span)` is bounded by 1,
degrades to the current ratio when the mean dominates, and reads as "how much of
this row's magnitude is the sweep". Whatever is chosen, `rtol * scale` in the
threshold on the line above must use the same scale, or the two disagree about what
counts as noise.
...
*Duplicate detection.* The declared sweep axis is ordered by construction; an
arbitrary row is not. A hysteresis loop passes the same gate voltage twice and
`argmin` would pick one silently. So `nearest_index` also warns when the value found
occurs at more than one point, naming the count and the indices. Tested on the
**coordinate** (`|values - found| <= _axis_atol`) rather than on the distance, so a
request landing midway between two *distinct* points does not trip it — that case is
a legitimate between-grid-points lookup, which the half-step rule above already
covers.
...
*Adjacent, not fixed.* `sweep="piezo_y"` on a raster gives a monotonic **staircase**
rather than a sawtooth — `n_fast` duplicate coordinates per point. It overplots too,
less catastrophically, and A8's warning does not fire on it. Left alone rather than
folded in unasked; worth a decision on whether duplicate sweep coordinates deserve
their own warning.
...
*Test:* `tests/test_loaders_nesting.py`, 45 cases. `make_spectral_csv` now derives
